...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        di = x - sample_mean
        inv_sd = 1 / np.sqrt(sample_var + eps)
        ...
        cache = (norm_x, sample_mean, sample_var, x, N, eps, gamma, beta, di, inv_sd)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    norm_x, sample_mean, sample_var, x, N, eps, gamma, beta, di, inv_sd = cache
    ...
    dsd = - np.sum(dnorm_x * di, axis=0) / (sample_var + eps)
    dvar = dsd / (2 * np.sqrt(sample_var + eps))

    ddi = dnorm_x * inv_sd + 2 * di * dvar * 1/N
...
def batchnorm_backward_alt(dout, cache):
    ...
    ddi = (- 1/N * di * np.sum(di * dnorm_x, axis =0) * inv_sd**2 + dnorm_x) * inv_sd

    dx = ddi - 1/N * np.sum(ddi, axis=0)  
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out = out.reshape((N*F, new_H, new_W))
    ...
    for h_index in range(new_H):
        for w_index in range(new_W):
            ...
            ans, _= affine_forward(x_selected, f_trans, b)
            
            out[range(N*F), [h_index]*(N*F), [w_index]*(N*F)] = ans.flatten()
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    ###########################################################################
    N, C, H, W = x.shape
    trans_x = np.transpose(x,(1,0,2,3)).reshape((C,-1)).T
    out, cache = batchnorm_forward(trans_x, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    trans_dout = np.transpose(dout,(1,0,2,3)).reshape((C,-1)).T
    dx, dgamma, dbeta = batchnorm_backward(trans_dout, cache)
